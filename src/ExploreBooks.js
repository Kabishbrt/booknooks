...
export const ExploreBooks = () => {
  const { status, isLoading, totalcount, books, error } = useSelector(
    (state) => state.books
  );
  const { all_products, filter_products, sorting_value } = useSelector(
    (state) => state.filter
  );
          ...
          <section className="product-view--sort">
            ...
            <div className="main-product">
              {status === 200 ? (
                ...
                <p>
                  Oops! Failed to load, Try again later.
                  <br />
